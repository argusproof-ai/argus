...
class _Report(FPDF):
    def header(self):
        ...
        self.cell(0, 9, _ascii("Automated Employment Decision Tool -- Bias Audit Summary"), ln=1)
...
def _attestation_section(pdf: "_Report", att: Dict):
...
def build_pdf(audit: Dict, meta: Optional[AuditMeta] = None,
    ...
    # Metadata
    _kv(pdf, "AEDT / tool", meta.tool_name)
    ...
    if attestation:
        _attestation_section(pdf, attestation)
...
def save_pdf(audit: Dict, path: str, meta: Optional[AuditMeta] = None,
    ...
    with open(path, "wb") as f:
        f.write(build_pdf(audit, meta, attestation))
    return path
